...
function ApiRequest() {
  ...
  return (
    <>
      <div>
        <input type="number" value={number} onChange={(e)=>{setNumber(e.target.value)}} style={{width:'30px'}}/>
        <button type="button" onClick={()=>handleRequestByID(number)}>불러오기다냥🐾</button>
        <button type="button" onClick={()=>setData(null)}>비우기다냥🐾</button>
        <button type="button" onClick={()=>handleRequestByIDAsync(number)}>불러오기다냥🐾</button>
        {data && (
          <>
          <br/>
            <textarea cols="70" rows="8" value={JSON.stringify(data, null, 2)} readOnly style={{resize:"none"}} />
            <p>{data.title}</p>
            <img src={data.thumbnailUrl} alt="thumnail" />
          </>
        )}
      </div>
    </>
