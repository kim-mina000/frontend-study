...
function CounterEffect() {
  ...
  // // 실행 시점: HTML DOM 렌더링 직후 (즉, 우리가 만든 컴포넌트가 화면에 나타난 상태 이후ㄴ)
  // useEffect(()=>{
  ...
  // // }); // 렌더링 될 때마다 실행됨
  // },[]); // 마운트 될 때만 실행됨 (= 처음 한번만 실행됨 사용예: 백엔드에 데이터 달라고 요청할때 등)
  ...
  return (
    <>
      ...
      <button onClick={()=>{
        ...
        // set함수는 비동기함수 setCount가 실행한걸 기다리지 않고 바로 다음으로 넘어가버림 -> 반영이 안되어서 1씩 차이가 발생
        // 2. 버튼 클릭 시 count 값이 제대로 반영 안됨(왜? set 함수는 비동기 함수)
        // 실행 시점이 다르므로 두가지 동작이 다르게 동작됨
